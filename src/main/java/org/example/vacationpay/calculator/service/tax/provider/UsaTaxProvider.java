...
public class UsaTaxProvider implements RegionTaxProvider {
    ...
    public BigDecimal getTaxRate(BigDecimal monthlySalary) {
        if (monthlySalary.compareTo(BigDecimal.valueOf(10000)) > 0) {
            return BigDecimal.valueOf(0.30); // 30% для высоких доходов
        }
        return BigDecimal.valueOf(0.22); // 22% стандартный налог
    }
}
